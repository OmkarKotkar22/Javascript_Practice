...
function checkEmail(str){
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if(!(
            ...
            span.innerHTML = "Invalid character in email ❌";
            span.style.color = "red";
            return;
        }
    }
    ...
    if(atCount != 1){
        span.innerHTML = "Email must contain only one '@' ❌";
        span.style.color= "red";
        return;
    }

    if(atIndex == 0 || atIndex == str.length-1){
        span.innerHTML = "'@' cannot be first or last ❌";
        span.style.color= "red";
        return;
    }

    if(dotIndex == -1){
        span.innerHTML = "'.' must be present after '@' ❌";
        span.style.color= "red";
        return;
    }

    if(dotIndex == atIndex + 1){
        span.innerHTML = "'.' cannot come immediately after '@' ❌";
        span.style.color= "red";
        return;
    }
